...
async def ask_question_with_pdf(bedrock_client, question: str, pdf_text: str):
    try:
        ...
        response = await body_builder(bedrock_client, userInput)  # Certifique-se de que body_builder é uma função assíncrona
...
async def definirDoc(bedrock_client: str, question: str, filenames: list):
    try:
...
async def manage_bacen_data(s3_client, question: str):
    bucket_name = 'chameleon-hackathon-dev'
    response = await s3_client.list_objects_v2(Bucket=bucket_name, Prefix='dadosBacen/')  # Certifique-se de que list_objects_v2 é uma coroutine
    if 'Contents' in response:
        ...
        for filename in filenames:
            ...
            await download_pdf_from_s3(s3_client, bucket_name, filename, download_path)  # Certifique-se de que download_pdf_from_s3 é uma coroutine
            pdf_text = await extract_text_from_pdf(download_path)  # Certifique-se de que extract_text_from_pdf é uma coroutine
            if pdf_text:
...
async def ask_question(s3_client, bedrock_client, question: str):
    bucket_name = 'chameleon-hackathon-dev'

    filenames = await bucket_contains_data(s3_client, bucket_name)
    ...
    if object == 'dadosBacen/':
        ...
        await download_pdf_from_s3(s3_client, bucket_name, object, 'arquivo.pdf')  # Certifique-se de que download_pdf_from_s3 é uma coroutine
